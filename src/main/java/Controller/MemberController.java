...
public class MemberController extends HttpServlet {
	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			// 로그인 요청
			case "/loginPro.me":
			    System.out.println("===[LOGIN] hit /loginPro.me ===");
			    System.out.println("method=" + request.getMethod() + ", pathInfo=" + request.getPathInfo());
			    System.out.println("param keys=" + java.util.Arrays.toString(request.getParameterMap().keySet().toArray()));
			    System.out.println("id=" + request.getParameter("id") + ", pass=" + request.getParameter("pass"));

			    // ✅ 1. 사용자가 보낸 아이디/비밀번호 찍기
			    String inputId = request.getParameter("id");
			    String inputPass = request.getParameter("pass");
			    System.out.println("[DEBUG] 입력받은 아이디: " + inputId);
			    System.out.println("[DEBUG] 입력받은 비밀번호: " + inputPass);

			    // ✅ 2. DB 조회 결과 찍기
			    int check = memberservice.serviceUserCheck(request);
			    System.out.println("[DEBUG] serviceUserCheck() 결과: " + check);
			    ...
			    System.out.println("[DEBUG] 세션에 저장 완료: id = " + session.getAttribute("id"));
			...
			case "/kakaoCallback.me":
				...
				try {
					...
					if (memberVo != null) {
						...
						response.sendRedirect(request.getContextPath() + "/member/loginPro.me");
			...
			if (nextPage != null) {
				...
				System.out.println("[REDIRECT] → " + request.getContextPath() + "/view/main");
