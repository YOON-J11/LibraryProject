			...
			<div class="swiper-wrapper">
				<c:if test="${not empty eventBannerList}">
					<c:forEach var="banner" items="${eventBannerList}">
						<div class="swiper-slide">
						  <c:choose>
						    <c:when test="${not empty banner.bannerImg}">
						      <a href="${pageContext.request.contextPath}/bbs/eventInfo.do?boardId=${banner.boardId}">
						        <img src="${banner.bannerImg}" alt="배너"
						             loading="lazy" style="display:block;max-width:100%;height:auto;" />
						      </a>
						    </c:when>
						    <c:otherwise>
						      <img src="${pageContext.request.contextPath}/resources/img/no-image.png" alt="배너 없음"
						           loading="lazy" style="display:block;max-width:100%;height:auto;" />
						    </c:otherwise>
						  </c:choose>
						</div>
